...
class PostController
{
    ...
    static public function postRegister($table, $data, $suffix)
    {
        ...
        if (isset($data['password_' . $suffix]) && $data['password_' . $suffix] != null) {
            ...
            if (isset($response['comment']) && $response['comment'] == 'todo bien') {
                /* ==============================================
                 Validar que el usuario exista en la base de datos
                 =============================================== */
                $response = GetModel::getDataFilter($table, '*', 'email_' . $suffix, $data['email_' . $suffix], null, null, null, null);

                // si viene diferente de vacio el usuario existe
                if (!empty($response)) {
                    /* ==================
                      generar token
                      ===================== */
                    ...
                    if (isset($update['comment']) && $update['comment'] == 'editado correctamente') {
                        ...
                        $return = new PostController();
                        $return->fncResponse(null, 'error al generar token', $suffix);
                    }
                } else {
                    // error al buscar el usuario
                    $return = new PostController();
                    $return->fncResponse(null, 'error al buscar usuario', $suffix);
                }
            } else {
                // error al registrar el usuario
                $return = new PostController();
                $return->fncResponse(null, 'error al registrar usuario', $suffix);
            }
        }
    }
